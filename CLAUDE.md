...
- **Self-scoped** endpoints derive the user from the token — they take **no** profile-id input. `GET /organizations` returns the caller's organizations and `GET /invitations` the caller's invitations, both keyed off `@CurrentUser`. (Do not accept a `profileId` that must equal the requester and then assert equality — the JWT `sub` is the single source of "who".) Profile reads (`GET /profiles/**`) need only a valid JWT (no role check).
