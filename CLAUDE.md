...
- **Translatable enums:** an enum whose values are shown to users (e.g. `ControlFormatStatus`) implements `shared.domain.model.valueobject.TranslatableEnum`, declaring a `messageKey()` (a plain string — no Spring in the domain). When such an enum is passed as a **message argument** (e.g. `super("error.control.format.cannot_resume", currentStatus)`), the shared `MessageResolver` detects it and substitutes its locale-resolved label (`enum.control.format.status.draft` → `Draft`/`Borrador`) before formatting — so users never see the internal value. Note this is independent of `@JsonValue`/`label()`, which only controls JSON serialization; `MessageFormat` would otherwise render an enum via `name()`.
