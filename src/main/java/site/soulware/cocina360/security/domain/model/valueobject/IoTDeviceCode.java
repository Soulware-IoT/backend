 ...
 * within the fleet; distinct from the surrogate {@link IoTDeviceId}.
 */
public record IoTDeviceCode(String value) implements ValueObject {

    private static final SecureRandom RANDOM = new SecureRandom();
    private static final String PREFIX = "COCINA-";
    private static final int SUFFIX_BYTES = 4;

    public IoTDeviceCode {
    ...
    public static IoTDeviceCode of(String value) {
        return new IoTDeviceCode(value);
    }

    /** Mints a new factory code, e.g. {@code COCINA-3F9A2B7C}. */
    public static IoTDeviceCode generate() {
        byte[] bytes = new byte[SUFFIX_BYTES];
        RANDOM.nextBytes(bytes);
        return new IoTDeviceCode(PREFIX + HexFormat.of().withUpperCase().formatHex(bytes));
    }
}
