...
public class ProfileQueryService {
    ...
    public List<ProfileResult> handle(ListProfilesByIdsQuery query) {
        List<ProfileId> ids = query.profileIds().stream().map(ProfileId::of).toList();
        return this.profileRepository.findAllById(ids).stream().map(ProfileResult::from).toList();
    }
}
